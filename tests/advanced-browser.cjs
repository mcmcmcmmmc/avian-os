...
(async()=>{
 ...
 await p.goto(base);await p.waitForFunction(()=>document.getElementById('case-photo').naturalWidth>0);
 ...
 await p.getByLabel('黄眉柳莺',{exact:true}).check();await p.getByLabel('淡眉柳莺',{exact:true}).check();await p.getByLabel('黄腰柳莺',{exact:true}).check();
 await p.getByLabel('褐柳莺',{exact:true}).click();assert.equal(await p.locator('[name="bird-answer"]:checked').count(),3);
 await p.getByLabel('暂不定种',{exact:true}).check();assert.equal(await p.locator('[name="bird-answer"]:checked').count(),1);
 await p.getByLabel('黄眉柳莺',{exact:true}).check();assert.equal(await p.getByLabel('暂不定种',{exact:true}).isChecked(),false);await p.getByLabel('淡眉柳莺',{exact:true}).check();
 ...
 await p.getByLabel('巨嘴柳莺',{exact:true}).check();await p.getByLabel('很确定',{exact:true}).check();await p.click('#submit-guess');assert.match(await p.textContent('#result-title'),/标注不同/);assert.match(await p.textContent('#reason-feedback'),/很确定/);
 await p.click('#next-case');assert.match(await p.textContent('#case-number'),/01 \/ 9/);
 await p.click('#observe-form');await p.getByLabel('暂不定种',{exact:true}).check();await p.click('#submit-guess');assert.match(await p.textContent('#result-stamp'),/尚未缩小/);
