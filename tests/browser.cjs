...
(async()=>{
 ...
 await page.goto('http://127.0.0.1:18876/');await page.screenshot({path:'evidence/desktop-entry.png',fullPage:true});
 ...
 const offline=await context.newPage();offline.on('pageerror',e=>errors.push(e.message));await context.setOffline(true);await offline.goto('file://'+path.resolve('芦汀观测站.html'));await offline.click('#start');await offline.waitForSelector('[data-signal]');assert.equal(await offline.evaluate(()=>avianDiagnostics().started),true);
