...
const storeKey='avian-os.encounters.v4';let records=[],idx=0,c,state,motionUntil=0,generation=0,hearGeneration=-1,playLimit=null,ctx=null,analyser=null,bins=null;
...
function tabs(){
 $('enc-cases').innerHTML=D.scenes.map((s,i)=>`<button data-enc-case="${i}" aria-current="${i===idx}">${String(i+1).padStart(2,'0')} · ${esc(s.title)}<small>${esc(s.area)}${records.some(r=>r.id===s.id)?' · 已记录':''}</small></button>`).join('');
...
function load(n){
 stopSound();idx=(n+D.scenes.length)%D.scenes.length;c=D.scenes[idx];state=D.begin();motionUntil=0;
 $('enc-location').textContent=c.area+' / 虚构情境';$('enc-title').textContent=c.title;$('enc-index').textContent=`${String(idx+1).padStart(2,'0')} / ${D.scenes.length}`;
...
async function listen(){
 ...
 try{
  ...
  playLimit=setTimeout(()=>{if(token===generation)stopSound('片段播放结束，可以重听，也可查看声音笔记。');},65000);
...
$('enc-submit').onclick=()=>{
 ...
 const titles={supported:'这次，多条线索走向了同一个答案。',narrowed:'你保留了它，再缩小一点范围。',incomplete:'方向可以保留，还缺一次关键观察。',open:'你保留了不确定，下一次观察有了方向。',reconsider:'有几条线索，需要重新放在一起看。'};
 $('enc-review-title').textContent=titles[result.verdict];$('enc-review-summary').textContent=`你的结论：${result.answer}。本次虚构遭遇设定为 ${c.answer}（${c.latin}）。这是场景内的推理复盘，不是真实照片的自动鉴定。`;
...
$('enc-retry').onclick=()=>{load(idx);view.scrollIntoView({block:'start'});};$('enc-next').onclick=()=>{load(idx+1);view.scrollIntoView({block:'start'});};
...
$('enc-export').onclick=()=>{const url=URL.createObjectURL(new Blob([JSON.stringify({format:'AVIAN-OS encounters v4',records},null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='AVIAN-OS-遭遇记录.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
...
function drawScene(now){
 ...
 const sky=g.createLinearGradient(0,0,w,h);sky.addColorStop(0,c.kind==='low'?'#8caaa0':'#b1b79a');sky.addColorStop(1,'#3b6655');g.fillStyle=sky;g.fillRect(0,0,w,h);
 ...
 if(c.kind==='low'){g.fillStyle='#718f80';g.beginPath();g.moveTo(w*.55,h*.65);g.lineTo(w,h*.58);g.lineTo(w,h);g.lineTo(w*.35,h);g.fill();g.strokeStyle='#c7d1b34d';g.lineWidth=1;for(let i=0;i<14;i++){const x=w*.56+(i*39)%(w*.5),y=h*.71+(i*11)%(h*.23);g.beginPath();g.moveTo(x,y);g.lineTo(x+34,y);g.stroke();}}
